...
open class RustKeychain {
    ...
    private func getEncryptionKeyData(keyIdentifier: String, canaryKeyIdentifier: String) -> (String?, String?) {
        ...
        DispatchQueue.global(qos: .background).sync {
            let key = getDataFromResult(keychain.queryKeychainForKey(key: keyIdentifier))
            let canary = getDataFromResult(keychain.queryKeychainForKey(key: canaryKeyIdentifier))
