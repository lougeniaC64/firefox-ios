...
open class RustKeychain {
    ...
    static var sharedClientAppContainerKeychain: RustKeychain {
        ...
        return RustKeychain(serviceName: baseBundleIdentifier, accessGroup: accessGroupIdentifier)
    }
    ...
    func updateKeychainKey(_ data: Data, key: String) -> OSStatus {
        let keychainQueryDictionary = getBaseKeychainQuery(key: key)
        let updateDictionary = [kSecValueData: data]

        return SecItemUpdate(keychainQueryDictionary as CFDictionary, updateDictionary as CFDictionary)
    }
