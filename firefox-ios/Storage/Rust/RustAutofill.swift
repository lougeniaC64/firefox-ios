...
public class RustAutofill {
    ...
    private let keychain = RustKeychain.sharedClientAppContainerKeychain
    ...
    public func decryptCreditCardNumber(encryptedCCNum: String?) -> String? {
        ...
        return keychain.decryptCreditCardNum(encryptedCCNum: encryptedCCNum)
    }
    ...
    public func getStoredKey(completion: @escaping (Result<String, NSError>) -> Void) {
        getKeychainData { (key, encryptedCanaryPhrase) in
    ...
    private func handleExpectedKeyAction(encryptedCanaryPhrase: String?,
        ...
        do {
            canaryIsValid = try self.keychain.checkCreditCardsCanary(
    ...
    private func getKeychainData(completion: @escaping (String?, String?) -> Void) {
        DispatchQueue.global(qos: .background).sync {
            let (key, encryptedCanaryPhrase) = self.keychain.getCreditCardKeyData()
    ...
    private func createKey() throws -> String {
        return try self.keychain.createCreditCardsKeyData()
    }
