...
public class RustLoginEncryptionKeys {
    ...
    private func updateKeychainKey(_ value: Data, key: String) {
    ...
    private func setKeychainKey(_ value: String, key: String) {
        ...
        let status = keychain.setKeychainKey(data, key: key)

        if status == errSecSuccess {
            return
        } else if status == errSecDuplicateItem {
            updateKeychainKey(data, key: key)
        } else {
...
public class RustLogins: LoginsProtocol, KeyManager {
    ...
    private func getEncryptionKeyData(rustKeys: RustLoginEncryptionKeys) -> (String?, String?) {
        var keychainData: [String?] = [nil, nil]

        DispatchQueue.global(qos: .background).sync {
            let keyResult = rustKeys.keychain.queryKeychainForKey(key: rustKeys.loginPerFieldKeychainKey)
            let canaryPhraseResult = rustKeys.keychain.queryKeychainForKey(key: rustKeys.canaryPhraseKey)

            for (index, item) in [keyResult, canaryPhraseResult].enumerated() {
                switch item {
                case .success(let result):
                    if let data = result {
                        keychainData[index] = String(data: data, encoding: .utf8)
                    }
                case .failure(let err):
        ...
        return (keychainData[0], keychainData[1])
    }
